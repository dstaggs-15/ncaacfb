...
async function init() {
  ...
  function getLogoHtml(teamName: string) {
    ...
    if (logo) return `<img src="${logo}" style="width:28px;height:28px;object-fit:contain;vertical-align:middle;margin-right:6px;" />`;
    return '';
  }
        ...
        const gamesHtml = seasonGames.length > 0
          ? seasonGames.map((g: any) => `
              <tr>
                ...
                <td style="padding:10px;border-bottom:1px solid #222;">${getLogoHtml(g.home_team)}${g.home_team}</td>
                <td style="padding:10px;border-bottom:1px solid #222;font-weight:bold;color:#D4A017;">${g.home_score ?? '-'} - ${g.away_score ?? '-'}</td>
                <td style="padding:10px;border-bottom:1px solid #222;">${getLogoHtml(g.away_team)}${g.away_team}</td>
                <td style="padding:10px;border-bottom:1px solid #222;color:#888;font-size:13px;">${g.is_playoff ? '🏆 Playoff' : g.is_rivalry ? '⚔️ Rivalry' : ''}</td>
              </tr>`).join('')
  ...
  app.innerHTML = `
    <main style="min-height:100vh;background:#0a0a0a;color:white;font-family:system-ui;padding:40px 20px;max-width:900px;margin:0 auto;">
